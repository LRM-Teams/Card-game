...
- **89 试玩**：`deploy/nginx-8088.conf`（`listen 8088` 反代到 3000）；一键脚本（需 root）：`sudo ./deploy/deploy-89.sh`（构建镜像、起容器、装 nginx 8088 并探活）。
...
- 容器内应用**只监听 3000**；目标映射为 **仅** `127.0.0.1:3000:3000`，8088 由 nginx 反代（见 `deploy/nginx-8088.conf`）。
- 错误示例：`-p 8088:8088`（2026-07-20 线上 502 事故）或长期 `-p 8088:3000` 直出应用（与网关目标不一致）。
