...
## 8. P0 关键动效（LRM-208 正式表 · 替换 LRM-168 interim）

> **完整可测表 + interim 对照 + 小林勾选清单**：[`docs/doudizhu-motion-spec.md`](./doudizhu-motion-spec.md)  
> 硬边界：单段动效 **≤ 700ms**；禁止大面积闪光 / 强金色泛光；出牌主按钮（`.btn.primary.cta`）保持唯一最高亮。  
> 时长表源码常量：`apps/client/src/lib/motionSpec.ts`（以 LRM-208 正式值为准）。

| 场景 | 动效规格 | vs interim / 备注 |
|---|---|---|
| 选牌 | 80–120ms 上移（LRM-196）+ 克制金色 glow；取消选择反向回落 | 保持 |
| 出牌飞入 | **260ms**（LRM-209，区间 180–320）自座位方向飞入出牌区 + 缩放；本席自下、左右席侧向；禁止瞬移 | LRM-209 已定，本单保持 |
| **叫分弹出** | 按钮区 **220ms** 弹入（`translateY 10→0`，`scale 0.96→1`）；禁止二次强闪光 | 正式写入（数值保持） |
| 倒计时 | 角落弱环；**≤3s 红色脉冲**（LRM-209：600ms 周期，仅 timer 本体，禁止全屏闪红）；**脉冲不得盖过主按钮亮度** | LRM-209 已定，本单保持 |
| **发牌散开** | **500ms**；stagger **24ms**；`scale 0.88→1`，`translateY 40→0`；无全屏闪光 | **替换** |
| **轮到谁（行动）** | 座位边框局部金色脉冲 **1000ms** 循环；头像环 scale **≤1.012**；glow alpha **0.12↔0.22**（弱于出牌 CTA） | **替换** |
| **身份常驻** | 角标静态 + 身份色环；禁止与行动共用同一套 glow | 保持 |
| **炸弹** | 局部爆点（半径 **≤64px**）+ `bomb.svg` + 轻震 **400ms**；禁止全屏闪光 | **替换** |
| **王炸** | 同炸弹档位、略强：**520ms**，爆点 **≤76px** | **替换** |
...
- LRM-208（2026-07-21，小雅）：§8 关键动效升格为正式表（替换 LRM-168 interim）；完整表见 `docs/doudizhu-motion-spec.md`。
