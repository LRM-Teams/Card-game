...
- 叫地主：**抢地主 A 方案**（`claim`=叫/抢，`pass`=不叫/不抢；最后一个 `claim` 为地主；全员不叫则重发，封顶后座位 0 当地主）。
- 地主拿底牌（共 20 张，底牌公开）。
...
socket.emit('action', { type: 'bid', choice: 'claim' }); // or choice: 'pass'
...
pnpm --filter @card-game/server simulate # 本地跑完 1 真人 + 2 机器人权威对局并输出结算日志
